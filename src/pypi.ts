...
export async function fetchPackument(pkg: string): Promise<Packument> {
  ...
  try {
    return (await getJson(`${PYPI_BASE}/${encodeURIComponent(name)}/json`)) as Packument;
  } catch (error) {
    if (error instanceof AxiError) throw error;
    if ((error as { status?: number }).status === 404) throw notFound(pkg);
...
export async function fetchVersion(pkg: string, version: string): Promise<VersionDoc> {
  const name = normalizeName(pkg);
  try {
    ...
    if ((error as { status?: number }).status === 404) {
      throw new AxiError(`version "${version}" of "${pkg}" not found on PyPI`, "NOT_FOUND", [
 ...
 * instead of an error.
 */
export async function fetchDownloadsRecent(pkg: string): Promise<DownloadsRecent | null> {
  const name = normalizeName(pkg);
  try {
    ...
    throw new AxiError("pypistats.org returned an unexpected error", "REGISTRY", [
      "Try again in a moment",
    ]);
  }
}
