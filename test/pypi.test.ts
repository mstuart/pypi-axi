...
import { fetchDownloadsRecent, fetchPackument, fetchVersion } from "../src/pypi.js";
...
describe("fetchPackument", () => {
  it("fetches and returns the packument", async () => {
    mockFetch({ "pypi.org/pypi/requests/json": { json: { info: { name: "requests" } } } });
  ...
  it("normalizes the package name (PEP 503) before building the URL", async () => {
    mockFetch({ "pypi.org/pypi/flask-cors/json": { json: { info: { name: "Flask-Cors" } } } });
...
describe("fetchVersion", () => {
  it("fetches a specific version document", async () => {
    mockFetch({
      "pypi.org/pypi/requests/2.31.0/json": { json: { info: { name: "requests", version: "2.31.0" } } },
    });
  ...
  it("translates a 404 into a NOT_FOUND AxiError naming the version", async () => {
    ...
    await expect(fetchVersion("requests", "9.9.9")).rejects.toMatchObject({ code: "NOT_FOUND" });
  });
});
...
describe("fetchDownloadsRecent", () => {
  it("returns the recent downloads data", async () => {
    mockFetch({
      "pypistats.org/api/packages/requests/recent": {
        json: { data: { last_day: 1, last_week: 2, last_month: 3 } },
      },
    });
    const recent = await fetchDownloadsRecent("requests");
    expect(recent).toEqual({ last_day: 1, last_week: 2, last_month: 3 });
  });
  ...
  it("still throws for a non-404 error status", async () => {
    mockFetch({ "pypistats.org/api/packages/requests/recent": { status: 500 } });
    await expect(fetchDownloadsRecent("requests")).rejects.toMatchObject({ code: "REGISTRY" });
  });
});
