...
const downloadsFixture = JSON.parse(
  readFileSync(
    fileURLToPath(
      new URL("../fixtures/requests-downloads.json", import.meta.url)
    ),
    "utf8"
  )
);
...
describe("downloadsCommand", () => {
  it("shapes a real captured pypistats response", async () => {
    mockFetch({
      "pypistats.org/api/packages/requests/recent": { json: downloadsFixture },
    });
    ...
    expect(out.help).toEqual([
      "Run `pypi-axi view requests` for package details",
    ]);
  });
  ...
  it("requires a package name", async () => {
    await expect(downloadsCommand([])).rejects.toMatchObject({
  ...
  it("rejects an unknown flag", async () => {
    await expect(
      downloadsCommand(["requests", "--bogus"])
